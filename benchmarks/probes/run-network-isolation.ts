...
for(const engine of ["remotion","hyperframes"]){
  const command=["-E","unshare","--net","--","sh","-c",'ip link set lo up && exec "$@"',"cinekernel-network-isolation",cargo,"xtask","phase0","run","--engine",engine,"--case","media-frame-sampling","--profile","smoke","--timeout-seconds","900","--json"];
  ...
  results.push({
    ...
    command:`sudo -E unshare --net -- sh -c 'ip link set lo up; exec ${cargo} xtask phase0 run --engine ${engine} ...'`,
