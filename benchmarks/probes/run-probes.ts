...
const expectedGroups:any[]=manifest.expected_groups??[];
const expectedEngines=new Set(expectedGroups.map((group:any)=>group.engine));
...
// Probe B: every frame was decoded/classified for five repetitions in every capability-declared mode.
const mediaEvidence=[];let mediaPass=true;
for(const engine of ["remotion","hyperframes"]){const modes=[...new Set(expectedGroups.filter((entry:any)=>entry.engine===engine&&entry.case_id==="media-frame-sampling").map((entry:any)=>String(entry.worker_mode)))];for(const mode of modes){const candidates=group(engine,"media-frame-sampling",mode);const checks=candidates.slice(0,5).map(entry=>entry.value.verification?.decoded?.media_oracle);const ok=candidates.length>=5&&checks.length===5&&checks.every(check=>check?.all_frames_match===true&&check?.checked_frame_count===180);mediaPass&&=ok;mediaEvidence.push({engine,worker_mode:mode,repetitions:candidates.length,required_repetitions:5,frames_checked:checks.map(check=>check?.checked_frame_count),all_match:ok,first_mismatches:checks.map(check=>check?.first_mismatch)})}}
...
const snapshotCases=[["remotion","media-frame-sampling"],["remotion","mixed-2d-3d"],["remotion","3d-scene"],["hyperframes","media-frame-sampling"],["hyperframes","mixed-2d-3d"],["hyperframes","3d-scene"]] as const;
const snapshotPositions=(caseId:string,totalFrames:number)=>{const boundaries=caseId==="mixed-2d-3d"?[Math.round(totalFrames*.2),Math.round(totalFrames*7/15),Math.round(totalFrames*.8)]:[];return[...new Set([Math.round(totalFrames*.1),Math.round(totalFrames*.5),Math.round(totalFrames*.9),boundaries[0]??0].map(frame=>Math.min(totalFrames-1,Math.max(0,frame))))]};
const remotionTasks=[];
for(const [engine,caseId] of snapshotCases){if(engine!=="remotion")continue;const candidate=group(engine,caseId,"default")[0];if(!candidate)continue;const durationSeconds=candidate.value.configuration.duration_seconds;for(const frame of snapshotPositions(caseId,Math.round(durationSeconds*30))){const destination=resolve(snapshotDir,engine,caseId,String(frame));mkdirSync(destination,{recursive:true});remotionTasks.push({case_id:caseId,duration_seconds:durationSeconds,frame,output:resolve(destination,"still.png")})}}
const remotionTaskFile=resolve(snapshotDir,"remotion-tasks.json");writeFileSync(remotionTaskFile,`${JSON.stringify(remotionTasks,null,2)}\n`);const remotionRunner=resolve(root,"packages/phase0-remotion/scripts/probe-stills.ts");const remotionTsx=resolve(root,"packages/phase0-remotion/node_modules/tsx/dist/cli.mjs");const remotionExecution=spawnSync(process.execPath,[remotionTsx,remotionRunner,"--","--tasks-file",remotionTaskFile,"--fixtures",fixtures,"--bundle-dir",resolve(snapshotDir,"remotion-bundle")],{cwd:root,encoding:"utf8",maxBuffer:128*1024*1024,env:{...process.env}});const remotionPayload=[...(remotionExecution.stdout??"").split(/\r?\n/)].reverse().map(line=>{try{return JSON.parse(line)}catch{return null}}).find(Boolean)??{results:[]};const remotionResults=new Map((remotionPayload.results??[]).map((result:any)=>[`${result.case_id}:${result.frame}`,result]));
const snapshotEvidence=[];let snapshotPass=true;
for(const [engine,caseId] of snapshotCases){
  ...
  const totalFrames=Math.round(candidate.value.configuration.duration_seconds*30);const comparisons=[];
  for(const frame of snapshotPositions(caseId,totalFrames)){const destination=resolve(snapshotDir,engine,caseId,String(frame));mkdirSync(destination,{recursive:true});let still="";let exitCode=1;let attempts:unknown[]=[];
    if(engine==="remotion"){const result=remotionResults.get(`${caseId}:${frame}`) as any;still=result?.output??resolve(destination,"still.png");exitCode=result?.exit_code??remotionExecution.status??1;attempts=result?.attempts??[{attempt:1,exit_code:exitCode,stderr:(remotionExecution.stderr??"").slice(-4000)}];}
    else{const project=resolve(root,".cinekernel/projects/hyperframes",`${canonicalRunId}-${caseId}-full`);const cli=resolve(root,"packages/phase0-hyperframes/node_modules/hyperframes/bin/hyperframes.mjs");const execution=spawnSync(process.execPath,[cli,"snapshot",project,"--output",destination,"--at",String(frame/30),"--no-end","--describe","false"],{cwd:root,encoding:"utf8",maxBuffer:128*1024*1024});exitCode=execution.status??1;still=walk(destination).find(path=>path.endsWith(".png"))??"";}
    const difference=exitCode===0&&still?mae(decodeRgb(still),decodeRgb(outputFor(candidate),frame)):Number.POSITIVE_INFINITY;const threshold=caseId==="media-frame-sampling"?9:engine==="remotion"&&caseId==="3d-scene"?20:15;const ok=difference<=threshold;snapshotPass&&=ok;comparisons.push({frame,time_seconds:frame/30,exit_code:exitCode,mae:difference,threshold,passed:ok,...(attempts.length?{attempts}:{})});
  }
