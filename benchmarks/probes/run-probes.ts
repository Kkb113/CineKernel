...
const pause=(milliseconds:number)=>Atomics.wait(new Int32Array(new SharedArrayBuffer(4)),0,0,milliseconds);
const probes:Probe[]=[];
...
for(const [engine,caseId] of [["remotion","media-frame-sampling"],["remotion","mixed-2d-3d"],["remotion","3d-scene"],["hyperframes","media-frame-sampling"],["hyperframes","mixed-2d-3d"],["hyperframes","3d-scene"]] as const){
  ...
  for(const frame of [...new Set(positions)]){const destination=resolve(snapshotDir,engine,caseId,String(frame));mkdirSync(destination,{recursive:true});let still="";let execution;const attempts=[];
    if(engine==="remotion"){still=resolve(destination,"still.png");const cli=resolve(root,"packages/phase0-remotion/node_modules/@remotion/cli/remotion-cli.js");const props=JSON.stringify({caseId,width:1920,height:1080,durationSeconds:candidate.value.configuration.duration_seconds});const remotionArgs=[cli,"still","src/index.tsx","CineKernelBenchmark",still,"--frame",String(frame),"--props",props,"--public-dir",fixtures,"--log","error"];const browserExecutable=process.env.CINEKERNEL_BROWSER_EXECUTABLE;if(browserExecutable)remotionArgs.push("--browser-executable",browserExecutable);const gl=process.env.CINEKERNEL_REMOTION_GL;if(gl)remotionArgs.push("--gl",gl);for(let attempt=1;attempt<=3;attempt++){execution=spawnSync(process.execPath,remotionArgs,{cwd:resolve(root,"packages/phase0-remotion"),encoding:"utf8",maxBuffer:128*1024*1024});attempts.push({attempt,exit_code:execution.status,stderr:(execution.stderr??"").slice(-4000)});if(execution.status===0)break;pause(2000)}pause(750);}
    ...
    const difference=execution.status===0&&still?mae(decodeRgb(still),decodeRgb(outputFor(candidate),frame)):Number.POSITIVE_INFINITY;const threshold=caseId==="media-frame-sampling"?9:engine==="remotion"&&caseId==="3d-scene"?20:15;const ok=difference<=threshold;snapshotPass&&=ok;comparisons.push({frame,time_seconds:frame/30,exit_code:execution.status,mae:difference,threshold,passed:ok,...(attempts.length?{attempts}:{})});
  }
