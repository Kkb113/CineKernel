...
Candidates are deliberately nonfinal: stable source maps, explicit time domains, typed diagnostics, capability-aware parity, immutable intent separated from materialization, streaming evaluation, version negotiation, and a procedural escape hatch over a finite renderer contract.
