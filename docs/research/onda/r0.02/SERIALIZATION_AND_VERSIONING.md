...
Scene uses JSON and carries a current version value of one, omitted for the current version. Future versions can be retained by the typed structure. Compatibility behavior for unknown fields and cross-version semantic guarantees needs an explicit contract and fixtures before CineKernel adopts a similar boundary.
