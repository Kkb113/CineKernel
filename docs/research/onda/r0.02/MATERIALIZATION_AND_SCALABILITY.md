...
React export constructs every Scene, serializes the complete array, then hands it to the native CLI. Motion blur multiplies the count. The structure predicts memory and I/O growth proportional to frame count and graph size. R0.02 does not benchmark ONDA, so thresholds remain open.
