...
| Render-time network | medium | critical | local source audit passes, but strong Ubuntu `unshare --net` Probe G is not dispatched | register manual workflow and require loopback-only sandbox for certification | security lead | Phase 1 entry | blocked |
