...
execFileSync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-y", "-framerate", "30", "-i", resolve(frames, "frame-%04d.ppm"), "-c:v", "libx264", "-preset", "medium", "-crf", "12", "-pix_fmt", "yuv420p", sourceVideo], {stdio: "inherit"});
...
for (let i = 0; i < samples; i++) {
  ...
  const window = t < 2 ? 440 : t >= 3 && t < 5 ? 660 : t >= 6 ? 880 : 0;
  const envelope = window === 0 ? 0 : Math.min(1, Math.min((t % 3) * 20, ((3 - (t % 3)) * 20)));
  const value = Math.round(Math.sin(2 * Math.PI * window * t) * 0.2 * envelope * 32767);
...
writeFileSync(resolve(out, "tone-windows.wav"), wav);
...
const assetNames = ["color-coded-source.mp4", "color-coded-oracle.json", "tone-windows.wav", "captions.vtt", "chart-data.json", "vector-fixture.svg", "texture.png", "cube.gltf", "font/inter-latin.woff2"];
const manifest = {
  schema_version: "phase0.fixtures.v1",
